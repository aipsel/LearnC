...
import xyz.imyeo.learnc.editor.EditTextPadding;
import xyz.imyeo.learnc.editor.LineUtils;
import xyz.imyeo.learnc.editor.PageSystem;
import xyz.imyeo.learnc.editor.Patterns;
...
public class CodeEditor extends EditText {
    ...
    private GoodScrollView mVerticalScroll;
    ...
    //region CONSTRUCTOR
    public CodeEditor(final Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public void setupEditor(GoodScrollView verticalScroll, PageSystem pageSystem) {
...
//        }
        ...
        setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (!PreferenceHelper.getReadOnly(getContext())) {
                    mVerticalScroll.tempDisableListener(1000);
                    ((InputMethodManager) getContext().getSystemService(Context
                            .INPUT_METHOD_SERVICE))
                            .showSoftInput(CodeEditor.this, InputMethodManager.SHOW_IMPLICIT);
                }
    ...
    public void updatePadding() {
        ...
        mVerticalScroll.setPadding(0, 0, 0, EditTextPadding.getPaddingBottom(context));
    }
...
//                    ((MainActivity) getContext()).saveTheFile(false);
    ...
    public Editable highlight(Editable editable, boolean newText) {
        ...
        if (!newText && editorHeight > 0) {
            firstVisibleIndex = getLayout().getLineStart(lineUtils.getFirstVisibleLine(mVerticalScroll, editorHeight, lineCount));
            lastVisibleIndex = getLayout().getLineEnd(lineUtils.getLastVisibleLine(mVerticalScroll, editorHeight, lineCount, deviceHeight) - 1);
        } else {
            firstVisibleIndex = 0;
        ...
        } else if (Arrays.asList(Utils.MIME_CODE).contains(fileExtension)) {
        ...
        } else if (Arrays.asList(Utils.MIME_SQL).contains(fileExtension)) {
            color(Patterns.SYMBOLS, editable, textToHighlight, firstColoredIndex);
            color(Patterns.GENERAL_STRINGS, editable, textToHighlight, firstColoredIndex);
            color(Patterns.SQL_KEYWORDS, editable, textToHighlight, firstColoredIndex);
        } else {
            if (!(Arrays.asList(Utils.MIME_MARKDOWN).contains(fileExtension)))
                color(Patterns.GENERAL_KEYWORDS, editable, textToHighlight, firstColoredIndex);
            color(Patterns.NUMBERS_OR_SYMBOLS, editable, textToHighlight, firstColoredIndex);
            color(Patterns.GENERAL_STRINGS, editable, textToHighlight, firstColoredIndex);
            if (fileExtension.equals("prop") || fileExtension.contains("conf") ||
                    (Arrays.asList(Utils.MIME_MARKDOWN).contains(fileExtension)))
    ...
    private final class EditTextChangeListener
        ...
        public void afterTextChanged(Editable s) {
            ...
            if (showUndo != mShowUndo || showRedo != mShowRedo) {
                mShowUndo = showUndo;
                mShowRedo = showRedo;
                // TODO update menu and code syntax.
